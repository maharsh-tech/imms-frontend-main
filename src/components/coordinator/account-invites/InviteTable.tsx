import { Trash2, UserPlus } from 'lucide-react'
import type { AccountInvite } from '../../../types'
import type { RoleFilter } from './account-invite-utils'

type InviteTableProps = {
  invites: AccountInvite[]
  roleFilter: RoleFilter
  prefixes: string[]
  selectedPrefix: string | null
  page: number
  totalPages: number
  totalInvites: number
  listBusy: boolean
  bulkDeletePending: boolean
  onRoleFilterChange: (role: RoleFilter) => void
  onPrefixFilterChange: (prefix: string) => void
  onPageChange: (page: number) => void
  onDelete: (id: string) => void
  onBulkDelete: () => void
  onOpenRoster: (invite: AccountInvite) => void
}

const InviteTable = ({
  invites,
  roleFilter,
  prefixes,
  selectedPrefix,
  page,
  totalPages,
  totalInvites,
  listBusy,
  bulkDeletePending,
  onRoleFilterChange,
  onPrefixFilterChange,
  onPageChange,
  onDelete,
  onBulkDelete,
  onOpenRoster,
}: InviteTableProps) => {
  const needsPrefixChoice = roleFilter === 'STUDENT' && selectedPrefix === null
  const bulkPrefix =
    roleFilter === 'STUDENT' && selectedPrefix && selectedPrefix !== 'ALL'
      ? selectedPrefix
      : null

  return (
    <>
      <div className="flex gap-2 mb-4">
        {(['STUDENT', 'TEACHER', 'COORDINATOR', 'ALL'] as RoleFilter[]).map((role) => (
          <button
            key={role}
            type="button"
            onClick={() => onRoleFilterChange(role)}
            className={`px-3 py-1 text-xs font-medium rounded-full ${
              roleFilter === role
                ? 'bg-primary text-white'
                : 'bg-background text-on-surface-variant hover:bg-gray-200'
            }`}
          >
            {role === 'ALL' ? 'All' : role.charAt(0) + role.slice(1).toLowerCase() + 's'}
          </button>
        ))}
      </div>

      {roleFilter === 'STUDENT' && (
        <div className="flex flex-wrap items-center gap-2 mb-4">
          <button
            type="button"
            onClick={() => onPrefixFilterChange('ALL')}
            className={`px-3 py-1 text-xs font-medium rounded-full ${
              selectedPrefix === 'ALL'
                ? 'bg-primary text-white'
                : 'bg-background text-on-surface-variant hover:bg-gray-200'
            }`}
          >
            All
          </button>
          {prefixes.map((prefix) => (
            <button
              key={prefix}
              type="button"
              onClick={() => onPrefixFilterChange(prefix)}
              className={`px-3 py-1 text-xs font-medium rounded-full ${
                selectedPrefix === prefix
                  ? 'bg-primary text-white'
                  : 'bg-background text-on-surface-variant hover:bg-gray-200'
              }`}
            >
              {prefix}
            </button>
          ))}
          {bulkPrefix && (
            <button
              type="button"
              onClick={onBulkDelete}
              disabled={listBusy || bulkDeletePending || totalInvites === 0}
              className="ml-auto px-3 py-1 text-xs font-semibold rounded-md border border-red-200 text-red-700 hover:bg-red-50 disabled:opacity-50"
            >
              Delete {bulkPrefix} accounts
            </button>
          )}
        </div>
      )}

      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-surface-variant">
          <thead className="bg-surface-container-low">
            <tr>
              <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant uppercase">ID</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant uppercase">Name</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant uppercase">Email</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant uppercase">Role</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant uppercase">Roster</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-on-surface-variant uppercase">Signed in</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-on-surface-variant uppercase">Actions</th>
            </tr>
          </thead>
          <tbody className="bg-surface-container-lowest divide-y divide-surface-variant">
            {needsPrefixChoice ? (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-center text-sm text-on-surface-variant">
                  Pick a batch to load student accounts.
                </td>
              </tr>
            ) : listBusy ? (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-center text-sm text-on-surface-variant animate-pulse">
                  Loading accounts...
                </td>
              </tr>
            ) : invites.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-6 text-center text-sm text-on-surface-variant">
                  No accounts in this category.
                </td>
              </tr>
            ) : (
              invites.map((invite) => (
                <tr key={invite.id} className="hover:bg-surface-container-low">
                  <td className="px-4 py-3 text-sm font-mono font-medium text-on-surface">
                    {invite.identifier ?? '—'}
                  </td>
                  <td className="px-4 py-3 text-sm text-on-surface">
                    {invite.name?.trim() ? invite.name : '—'}
                  </td>
                  <td className="px-4 py-3 text-sm font-mono text-on-surface">{invite.email}</td>
                  <td className="px-4 py-3 text-sm">
                    <span
                      className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                        invite.role === 'COORDINATOR'
                          ? 'bg-purple-100 text-purple-800'
                          : invite.role === 'TEACHER'
                            ? 'bg-green-100 text-green-800'
                            : 'bg-blue-100 text-primary'
                      }`}
                    >
                      {invite.role}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm">
                    {invite.role === 'COORDINATOR' ? (
                      <span className="text-outline">—</span>
                    ) : invite.rosterLinked ? (
                      <span className="text-green-700 font-medium">In roster</span>
                    ) : (
                      <span className="text-amber-700 font-medium">Not in roster</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-sm">
                    {invite.hasSignedIn ? (
                      <span className="text-green-700 font-medium">Yes</span>
                    ) : (
                      <span className="text-on-surface-variant">Not yet</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right text-sm space-x-2">
                    {invite.role === 'STUDENT' && !invite.rosterLinked && invite.identifier && (
                      <button
                        type="button"
                        onClick={() => onOpenRoster(invite)}
                        className="inline-flex items-center text-green-700 hover:text-green-900 text-xs font-medium"
                      >
                        <UserPlus className="w-4 h-4 mr-1" />
                        Add to roster
                      </button>
                    )}
                    <button
                      type="button"
                      onClick={() => onDelete(invite.id)}
                      className="inline-flex items-center text-red-600 hover:text-red-800 p-1"
                      title="Revoke"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </td>
                </tr>
              ))
            )}
          </tbody>
        </table>
        {!needsPrefixChoice && !listBusy && totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-outline-variant bg-surface-container-low text-sm">
            <span className="text-on-surface-variant">
              Page {page} of {totalPages} · {totalInvites} accounts
            </span>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => onPageChange(page - 1)}
                disabled={page <= 1 || listBusy}
                className="px-3 py-1 rounded-md border border-outline-variant disabled:opacity-50"
              >
                Previous
              </button>
              <button
                type="button"
                onClick={() => onPageChange(page + 1)}
                disabled={page >= totalPages || listBusy}
                className="px-3 py-1 rounded-md border border-outline-variant disabled:opacity-50"
              >
                Next
              </button>
            </div>
          </div>
        )}
      </div>
    </>
  )
}

export default InviteTable
